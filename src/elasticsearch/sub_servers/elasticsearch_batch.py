...
async def batch_index_directory(
    ...
    """Batch index all documents from a directory into Elasticsearch with comprehensive processing and AI enhancement."""
    try:
        ...
        for file_path in valid_files:
            try:
                ...
                # Use AI enhancement if requested and content is available
                if use_ai_enhancement and content.strip():
                    try:
                        # Note: AI enhancement would require context parameter
                        # For now, we'll use basic enhancement patterns
                        # In full implementation, this would call generate_smart_metadata
                        
                        # Basic content analysis for tags
                        content_lower = content.lower()
                        if any(word in content_lower for word in ['class', 'function', 'def', 'import']):
                            final_tags.append("code")
                        if any(word in content_lower for word in ['config', 'setting', 'parameter']):
                            final_tags.append("configuration")
                        if any(word in content_lower for word in ['# ', '## ', '### ']):
                            final_tags.append("documentation")
                        if any(word in content_lower for word in ['test', 'example', 'demo']):
                            final_tags.append("example")
                        ...
                        # Add content-based key points
                        lines = content.split('\n')
                        non_empty_lines = [line.strip() for line in lines if line.strip()]
                        if non_empty_lines:
                            final_key_points.append(f"Lines of content: {len(non_empty_lines)}")
                        
                        # Look for headers/titles in markdown
                        headers = [line for line in non_empty_lines if line.startswith('#')]
                        if headers:
                            final_key_points.append(f"Contains {len(headers)} headers/sections")
                                
                    except Exception as e:
                        # AI enhancement failed, continue with basic metadata
                        pass
